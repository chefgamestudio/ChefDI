...
namespace gs.ChefDI.Unity
{
    ...
    public partial class LifetimeScope : MonoBehaviour, IDisposable
    {
        ...
        public bool IsRoot => ChefDISettings.Instance != null &&
                              ChefDISettings.Instance.IsRootLifetimeScopeInstance(this);

        readonly List<IInstaller> localExtraInstallers = new List<IInstaller>();

        protected virtual void Awake()
        {
            if (ChefDISettings.DiagnosticsEnabled && string.IsNullOrEmpty(scopeName))
            {
            ...
            catch (ChefDIParentTypeReferenceNotFound) when(!IsRoot)
            {
        ...
        public void Build()
        {
            ...
            if (Parent != null)
            {
                if (ChefDISettings.Instance != null && Parent.IsRoot)
                {
                ...
                Parent.Container.CreateScope(builder =>
                {
                    ...
                    builder.Diagnostics = ChefDISettings.DiagnosticsEnabled ? DiagnositcsContext.GetCollector(scopeName) : null;
                    InstallTo(builder);
                });
            }
            else
            {
                var builder = new ContainerBuilder
                {
                    ...
                    Diagnostics = ChefDISettings.DiagnosticsEnabled ? DiagnositcsContext.GetCollector(scopeName) : null,
                };
                builder.RegisterBuildCallback(SetContainer);
                InstallTo(builder);
                builder.Build();
        ...
        LifetimeScope GetRuntimeParent()
        {
            ...
            if (parentReference.Type != null && parentReference.Type != GetType())
            {
                ...
                throw new ChefDIParentTypeReferenceNotFound(
            ...
            if (ChefDISettings.Instance != null)
            {
                return ChefDISettings.Instance.GetOrCreateRootLifetimeScopeInstance();
            }
