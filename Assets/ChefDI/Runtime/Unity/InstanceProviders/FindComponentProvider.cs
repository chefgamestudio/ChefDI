    ...
    sealed class FindComponentProvider : IInstanceProvider
    {
        ...
        public object SpawnInstance(IObjectResolver resolver)
        {
            ...
            if (parent != null)
            {
                ...
                if (component == null)
                {
                    throw new ChefDIException(componentType, $"{componentType} is not in the parent {parent.name} : {this}");
                }
            }
            else if (scene.IsValid())
            {
                ...
                if (component == null)
                {
                    throw new ChefDIException(componentType, $"{componentType} is not in this scene {scene.path} : {this}");
                }
            }
            else
            {
                throw new ChefDIException(componentType, $"Invalid Component find target {this}");
            }
